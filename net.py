...
class ConvBNActivation(nn.Sequential):
    def __init__(
        ...
        super(ConvBNReLU, self).__init__(
            _Conv2d(in_planes, out_planes, kernel_size, stride, padding, dilation=dilation, groups=groups,
                      bias=False),
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, input_channels: int, squeeze_factor: int = 4):
        ...
        self.fc1 = _Conv2d(input_channels, squeeze_channels, 1)
        self.relu = nn.ReLU(inplace=True)
        self.fc2 = _Conv2d(squeeze_channels, input_channels, 1)
...
class MobileNetV3(nn.Module):
    ...
    def __init__(
        ...
        if norm_layer is None:
            norm_layer = partial(_BatchNorm2d, eps=0.001, momentum=0.01)
        ...
        for m in self.modules():
            ...
            elif isinstance(m, (_BatchNorm2d, nn.GroupNorm)):
...
class ReversiNet(nn.Module):
    def __init__(self, device='cpu', cfg=[(8, 128)], train=False):
        ...
        last_c = 960
        Conv2d = _Conv2d if train else _Conv2d
        Linear = _Linear if train else nn.Linear
        self.resblocks = mobilenet_v3_large(True)
        ...
        self.out2 = nn.Sequential(Conv2d(last_c, 1, 1), nn.Flatten())
        self.device = device
        self.to(device)
    ...
    def forward(self, x):
        ...
        x = self.quant(x)
        x = self.resblocks(x)
        p = self.dequant1(self.out1(x))
        q = self.dequant2(self.out2(x))
